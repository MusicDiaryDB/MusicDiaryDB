...
from helpers import create_user_resource, get_resource, handle_request, execute_query, delete_resource_with_multiple_keys, update_resource, admin_required
...
def create_admin_user() -> Any:
    # Extracting user details first from the request
    user_data = request.get_json()
    if not user_data:
        return jsonify({'error': 'Invalid JSON data'}), 400
    ...
    password = user_data.get('password')
    visibility = user_data.get('visibility')
    is_admin = True  # Admin endpoint, so we're creating an admin user
